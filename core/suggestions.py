"""Suggested calendar items the user can accept or dismiss: work sessions before task deadlines, and
deadlines / reply requests found in unread emails.

A suggestion is {ref, kind: study|deadline|reply, title, date, start, end, all_day, details}. Refs are
stable across reloads, so dismissing one (data/dismissed_suggestions.json) or accepting it (a saved
event carrying the ref) keeps it from coming back. Dismissing a study ref drops the whole task.
Study sessions are placed by core/scheduler.py, with estimates learned by core/estimates.py.
"""
...
from core.estimates import Estimate, describe, learn
...
def _study_details(reasons: tuple[str, ...], estimate: Estimate) -> str:
    return ". ".join([*reasons, estimate.note()]) + "."
...
def build_suggestions(
    ...
    """Returns {"items": [suggestion], "notices": [str], "estimates": [str]}.

    `events` must include saved (accepted) events. "estimates" describes what's been learned about task length.
    """
    ...
    for event in events:
        ...
        if ref.startswith(STUDY_PREFIX) and isinstance(start, datetime) and isinstance(end, datetime):
            minutes = _minutes(start, end)
            key = ref[len(STUDY_PREFIX):]
            accepted_minutes[key] = accepted_minutes.get(key, 0) + minutes
    ...
    candidates = study_candidates(tasks, today, study, accepted_minutes, dismissed_keys, calibration)
    ...
    for session in plan.sessions:
        ...
        details = _study_details(session.reasons, candidate["estimate_info"])
    ...
    for candidate in candidates:
        ...
        if short >= 15:
            title = str(candidate["task"].get("title", "Untitled task"))
            notices.append(f"Not enough free time before {candidate['due']:%a %b %d} for “{title}” ({short} min short).")
    ...
    return {"items": items, "notices": notices, "estimates": describe(calibration) if calibration else []}
